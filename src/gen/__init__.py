...
def generate_context_prompt(user_prompt :str, last_output = None, last_sql_result = None) -> str:
    ...
    if last_sql_result:
        main_prompt = f"""
            ...
            The last MySQL query generated was:
            ```sql
            {last_output}
            ```
            ...
            - If this value is an error or looks erroneous (or if it's 0 or NULL when expecting a value) then return an improved SQL Query with nothing but valid SQL, your entire response will be passed to the DB.

            The columns in the db are as follows: {generate_minimized_schema_prompt()}
        """
    else:
        main_prompt = f"""
            A user prompt will be provided below the system prompt,
